...
export const Paginator: FC<Props> = (props) => {
    ...
    const lastPortionPage =
        portion >= lastPortion ? totalPages : (portion + 1) * maxPage
    ...
    const handlerClickPageCallBack =
        (isCurrentPage: boolean, page: number) =>
    ...
    const buttonRight = (
        <button className={cl.btn} onClick={handlerClickButtonRight}>
            {'>'}{' '}
        </button>
    )
    const buttonLeft = (
        <button className={cl.btn} onClick={handlerClickButtonLeft}>
            {'<'}{' '}
        </button>
    )
    ...
    const mappedPages = pages.map((p, i) => {
        ...
        const className =
            cl.numberPage + ' ' + (isCurrentPage ? cl.currentPage : '')
        return (
            <li
                key={i}
                className={className}
    ...
    return (
        <div className={cl.pagesWrapper}>
            ...
            <ul className={cl.pages}>{mappedPages}</ul>
