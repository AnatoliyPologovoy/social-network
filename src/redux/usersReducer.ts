...
export const usersReducer = (
    ...
    switch (action.type) {
        ...
        case SET_USER_IN_FOLLOWING_PROGRESS:
            return {
                ...state,
                inFollowingProgressUsers: action.payload.isFetching
                    ? [...state.inFollowingProgressUsers, action.payload.userId]
                    : state.inFollowingProgressUsers.filter(
...
export const setUserInFollowingProgressAC = (
    userId: number,
    isFetching: boolean,
) => {
    return {
...
type setUserInFollowingProgressACType = ReturnType<
    typeof setUserInFollowingProgressAC
>
...
//Thunks
export const getUsersThunkCreator = (currentPage: number = 1): AppThunk => {
